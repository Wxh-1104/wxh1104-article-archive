...
const articles = articleFiles.map(file => {
    ...
    return {
        ...
        date: attributes.date ? dayjs(attributes.date).format('YYYY/MM/DD') : dayjs().format('YYYY/MM/DD'),
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: monospace; line-height: 1.5; padding: 1rem; background: #f5f5f5; }
...
<body>
  ...
  `;
    fs.writeFileSync(path.join(outputDir, articlesDir, `${article.filename}.html`), articleHtmlTemplate);
});

// 更新主页 (public/index.html)
let indexHtmlContent = fs.readFileSync(path.join(outputDir, 'index.html'), 'utf8');
...
if (articles.length > 0) {
    indexHtmlContent = indexHtmlContent.replace(cardContainerRegex, `<div class="card-container">\n${articleCardsHtml}\n</div>`);
} else {
    // 如果没有文章，可以显示提示信息或保留原始的示例卡片
    indexHtmlContent = indexHtmlContent.replace(cardContainerRegex, `<div class="card-container"><p>暂无文章。</p></div>`);
}
